...
順序：
    ...
    3. classify_quote_vs_remix.py   引用 vs 改編（需 lines_with_embedding.parquet）
    4. scene_heatmap.py             場景熱度排行
    5. judge.py                     產生 review_for_judge.md（給 Claude Code 讀）
...
Usage:
    ...
    uv run final_project/run_all.py --only cluster analyze   # 只跑指定的 step
"""
import sys
import argparse
import subprocess
...
def main():
    ...
    if not lines_path.exists():
        ...
        print(f"   classify_quote_vs_remix 需要這個檔案")
